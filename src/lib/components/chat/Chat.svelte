<script lang="ts">
	...
	import { getRealtimeClientConfig, type RealtimeClientConfig } from './MessageInput/realtime/api';
	...
	const resolveVoiceOverlayMode = async (
		...
		if (!realtimeConfig || realtimeClientConfigLoadStatus === 'failed') {
			...
			if (!realtimeConfig) {
				toast.error(
					$i18n.t('Voice session availability could not be confirmed. Please try again.')
				);
				return null;
			}
		}
	...
	const chatCompletedHandler = async (_chatId, modelId, responseMessageId, messages) => {
		...
		if ($chatId == _chatId && !$temporaryChatEnabled && !useRealtimeOverlay()) {
			await refreshChatList(localStorage.token);
		}
	};
