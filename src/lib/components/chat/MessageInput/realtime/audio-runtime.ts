...
export function createRealtimeAudioRuntime(options: AudioRuntimeOptions) {
	...
	let remotePlaybackSuppressed = false;
	...
	function suppressRemotePlayback(audioElement: HTMLAudioElement | null) {
		remotePlaybackSuppressed = true;
		stopRemoteQuietTimeout();
		assistantAudioActive = false;
		options.onAssistantAudioQuiet();
		if (audioElement) {
	...
	function restoreRemotePlayback(audioElement: HTMLAudioElement | null) {
		if (!remotePlaybackSuppressed) {
			return;
		}
