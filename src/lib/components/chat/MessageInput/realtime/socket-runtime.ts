...
type SocketRuntimeOptions = {
	...
	getCurrentResponseId: () => string;
	getInterruptingResponseId: () => string;
	...
	setCurrentResponseId: (value: string) => void;
	setInterruptingResponseId: (value: string) => void;
	...
	setVoiceSessionId: (value: string) => void;
	setVoiceGeneration: (value: number | null) => void;
...
export function setupRealtimeSocketRuntime(options: SocketRuntimeOptions) {
	...
	function getEventResponseId(data: any): string {
		return data?.responseId || data?.data?.responseId || data?.response?.id || '';
	}
	...
	function handleRealtimeEvents(event: any) {
		...
		if (type === 'status') {
			...
			const isContextActivity =
				desc.includes('preparing') ||
				desc.includes('context') ||
				desc.includes('loading') ||
				desc.includes('summariz') ||
				desc.includes('injecting');
	...
	function onRealtimeSession(data: any) {
		...
		if (typeof data?.voiceSessionId === 'string' && data.voiceSessionId) {
			options.setVoiceSessionId(data.voiceSessionId);
		}
		if (typeof data?.generation === 'number') {
			options.setVoiceGeneration(data.generation);
		}
		options.setPTTMode(
			data?.sessionConfig?.vadType === 'push_to_talk' || data?.sessionConfig?.vadType === null
		);
		options.setPTTHeld(false);
		options.setCurrentResponseId('');
		options.setInterruptingResponseId('');
		options.restoreAssistantPlayback();
		options.setAssistantSpeaking(false);
	...
	function onRealtimeResponseState(data: { callId?: string; responseId?: string; phase?: string }) {
		if (!matchesCurrentCall(data)) {
			return;
		}
		const responseId = getEventResponseId(data);
		...
		if (phase === 'started') {
			const interruptingResponseId = options.getInterruptingResponseId();
			options.setCurrentResponseId(responseId);
			options.restoreAssistantPlayback();
			if (interruptingResponseId && interruptingResponseId !== responseId) {
				options.setInterruptingResponseId('');
			}
			return;
		}
		...
		if (phase === 'cancel_requested') {
			options.setInterruptingResponseId(responseId);
			return;
		}

		if (phase === 'done') {
			if (options.getCurrentResponseId() === responseId) {
				options.setCurrentResponseId('');
			}
			if (options.getInterruptingResponseId() === responseId) {
				options.setInterruptingResponseId('');
				if (!options.getAssistantAudioActive()) {
					options.setAssistantSpeaking(false);
				}
	...
	options.socket.on('realtime:response_state', onRealtimeResponseState);
	...
	return () => {
		...
		options.socket.off('realtime:response_state', onRealtimeResponseState);
