<script lang="ts">
	...
	function resumeCheckin() {
		...
		if (checkinPaused) {
			checkinPaused = false;
			$socket?.emit('realtime:start_checkin', {
				callId,
				voiceSessionId,
			...
			try {
				...
				if (settingsCompression || configWidth || configHeight) {
					...
					let height = settingsCompression
						? ($settings?.imageCompressionSize?.height ?? null)
						: null;
				...
				if (files.some((f) => f.type !== 'image')) {
					toast.info(
						$i18n.t(
							'Voice mode only sends images to the model. Other attached files are saved to chat history but the assistant will not see them.'
						)
					);
				}
				submitPrompt('', files);
	...
	function handleVisibilityChange() {
		...
		if (document.hidden && isCameraOpen) {
			checkinPaused = true;
			$socket?.emit('realtime:stop_checkin', {
				callId,
				voiceSessionId,
				generation: voiceGeneration
			});
	...
	function pttStart() {
		...
		audioRuntime.stopToolPing();
		if (callId) {
			$socket?.emit('realtime:clear_audio', {
				callId,
				voiceSessionId,
				generation: voiceGeneration
			});
	...
	function pttEnd() {
		...
		if (callId) {
			$socket?.emit('realtime:commit_audio', {
				callId,
				voiceSessionId,
				generation: voiceGeneration
			});
