<script lang="ts">
	...
	import { config, models, settings, showCallOverlay, socket } from '$lib/stores';
	...
	function openImagePicker() {
		...
		isCameraOpen = true;
		checkinPaused = true;
		$socket?.emit('realtime:stop_checkin', { callId });
		fileInputEl?.click();
	}
	...
	function resumeCheckin() {
		...
		if (checkinPaused) {
			checkinPaused = false;
			$socket?.emit('realtime:start_checkin', { callId });
		}
	}
				...
				on:click={() => {
					if (isAssistantSpeaking || interruptingResponseId) {
						if (callId) {
							...
							$socket?.emit('realtime:cancel', { callId, responseId });
						}
