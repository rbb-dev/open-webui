<script lang="ts">
	...
	import { config, models, selectedFolder, settings, showCallOverlay, socket } from '$lib/stores';
	import { onMount, getContext, onDestroy, createEventDispatcher, tick } from 'svelte';
	...
	import { negotiateRealtimeSdp } from './realtime/api';
	import { toast } from 'svelte-sonner';
	import MicSolid from '$lib/components/icons/MicSolid.svelte';
	import Camera from '$lib/components/icons/Camera.svelte';
	import { convertHeicToJpeg, compressImage } from '$lib/utils';
	...
	function openImagePicker() {
		if (connectionState !== 'connected' || isContextPreparing) return;
		isCameraOpen = true;
		checkinPaused = true;
		$socket?.emit('realtime:stop_checkin', { callId, voiceSessionId, generation: voiceGeneration });
		fileInputEl?.click();
	}
	...
	function resumeCheckin() {
		...
		if (checkinPaused) {
			checkinPaused = false;
			$socket?.emit('realtime:start_checkin', { callId, voiceSessionId, generation: voiceGeneration });
		}
	}
	...
	async function handleImageSelected(event: Event) {
		...
		if (!file.type.startsWith('image/')) {
			toast.error($i18n.t('Please select an image file.'));
			resumeCheckin();
			return;
		}
		...
		reader.onload = async () => {
			try {
				...
				// Compress if settings or config require it (same as MessageInput)
				...
				await tick();
				await tick();
				if (files.some((f) => f.type !== 'image' && f.status === 'uploading')) {
					toast.error(
				...
				if (files.some((f) => f.type !== 'image')) {
					toast.info(
						$i18n.t('Voice mode only sends images to the model. Other attached files are saved to chat history but the assistant will not see them.')
					);
				}
				submitPrompt('', files);
				files = [];
			} catch {
				toast.error($i18n.t('Failed to process the selected image.'));
			} finally {
				resumeCheckin();
			}
		};
		reader.onerror = () => {
			toast.error($i18n.t('Failed to read the selected image.'));
			resumeCheckin();
		};
		// Convert HEIC to JPEG before reading (same as MessageInput)
		try {
			reader.readAsDataURL(file.type === 'image/heic' ? await convertHeicToJpeg(file) : file);
		} catch {
			toast.error($i18n.t('Failed to convert the selected image.'));
			resumeCheckin();
		}
	}
	...
	function handleVisibilityChange() {
		...
		if (document.hidden && isCameraOpen) {
			checkinPaused = true;
			$socket?.emit('realtime:stop_checkin', { callId, voiceSessionId, generation: voiceGeneration });
		} else if (!document.hidden && checkinPaused) {
			checkinPaused = false;
			$socket?.emit('realtime:start_checkin', { callId, voiceSessionId, generation: voiceGeneration });
		}
	}
	...
	function suppressAssistantPlayback() {
		audioRuntime.suppressRemotePlayback(audioElement);
		audioRuntime.stopToolPing();
		isAssistantSpeaking = false;
	}

	function restoreAssistantPlayback() {
		audioRuntime.restoreRemotePlayback(audioElement);
	}
		...
		try {
			...
			nextPc.onconnectionstatechange = () => {
				if (nextPc.connectionState === 'connected') {
					...
					connectionState = 'error';
					if (connectCallId) {
						$socket?.emit('realtime:stop', { callId: connectCallId, voiceSessionId, generation: voiceGeneration });
					}
	...
	function pttStart() {
		...
		audioRuntime.stopToolPing();
		if (callId) {
			$socket?.emit('realtime:clear_audio', { callId, voiceSessionId, generation: voiceGeneration });
		}
	...
	function pttEnd() {
		...
		if (callId) {
			$socket?.emit('realtime:commit_audio', { callId, voiceSessionId, generation: voiceGeneration });
		}
	}
	...
	onMount(async () => {
		if ($socket) {
			cleanupSocketRuntime = setupRealtimeSocketRuntime({
				...
				setCurrentResponseId: (value) => {
					currentResponseId = value;
				},
				setInterruptingResponseId: (value) => {
					interruptingResponseId = value;
				},
				...
				setVoiceSessionId: (value) => {
					voiceSessionId = value;
				},
				setVoiceGeneration: (value) => {
	...
	onDestroy(() => {
		...
		if (callId) {
			$socket?.emit('realtime:stop', { callId, voiceSessionId, generation: voiceGeneration });
		}
	...
	<div class="max-w-lg w-full h-full max-h-[100dvh] flex flex-col justify-between p-3 md:p-6">
		<div class="flex justify-center items-center flex-1 h-full w-full max-h-full">
			<button
				type="button"
				on:click={() => {
					if (isAssistantSpeaking || interruptingResponseId) {
						if (callId) {
							const responseId = currentResponseId || undefined;
							$socket?.emit('realtime:cancel', { callId, responseId, voiceSessionId, generation: voiceGeneration });
						}
						interruptingResponseId = currentResponseId || interruptingResponseId;
						suppressAssistantPlayback();
					}
				}}
			>
				{#if interruptingResponseId || isAssistantSpeaking || isToolCalling || isContextPreparing || connectionState === 'connecting'}
					<svg
		...
		<div class="flex justify-between items-center pb-2 w-full">
			<div class="flex gap-2">
				...
				{:else}
					...
					<button
						class="p-3 rounded-full relative flex items-center justify-center {isContextPreparing ||
						connectionState === 'connecting'
							? 'bg-gray-200 dark:bg-gray-800 opacity-50 cursor-not-allowed'
							: 'bg-gray-50 dark:bg-gray-900'}"
						type="button"
						disabled={isContextPreparing || connectionState === 'connecting'}
						aria-label={isContextPreparing || connectionState === 'connecting'
							? $i18n.t('Preparing...')
							: isMicMuted
								? $i18n.t('Unmute microphone')
								: $i18n.t('Mute microphone')}
						on:click={() => {
				...
				<button
					class="p-3 rounded-full {isContextPreparing || connectionState !== 'connected'
						? 'bg-gray-200 dark:bg-gray-800 opacity-50 cursor-not-allowed'
						: 'bg-gray-50 dark:bg-gray-900'}"
					type="button"
					aria-label={$i18n.t('Send photo')}
					disabled={isContextPreparing || connectionState !== 'connected'}
