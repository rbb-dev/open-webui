...
async def ensure_realtime_voice_turn(session: RealtimeSession, sio, item_id: str = '') -> VoiceTurn | None:
    ...
    if item_id and active_turn and not active_turn.has_any_output():
        log.debug(
            ...
            session.chat_id, item_id, active_turn.turn_id, active_turn.input_item_ids,
        )
    ...
    log.debug(
        ...
        session.chat_id, item_id, turn_id, parent_id, user_message_id, assistant_message_id,
    )
                ...
                if parent_msg:
                    ...
                    if user_msg_id not in children:
                        ...
                        log.debug(
                            ...
                            chat_id, parent_id, user_msg_id, siblings_before, siblings_before + 1,
                        )
...
async def _remove_orphan_turn_messages_locked(
...
async def _persist_turn_output_locked(session: RealtimeSession, turn: VoiceTurn | None) -> None:
    ...
    log.debug(
        ...
        session.chat_id, turn.assistant_message_id, current_id, sorted(a for a in branch_anchors if a), advanced,
    )
    if not advanced:
...
async def cleanup_realtime_voice_turns(session: RealtimeSession, sio=None, reason: str = 'user') -> None:  # noqa: C901 — per-turn finalize loop with three terminal cases (orphan / output-present / placeholder); decomposition would obscure the linear lifecycle read
    ...
    """
    if (
        not session.turn_state.turns
        or session.chat_id.startswith('local:')
        or session.chat_id.startswith('channel:')
    ):
        return
    ...
    async with chat_write_lock(session.chat_id):
        ...
        for turn in list(session.turn_state.turns.values()):
            ...
            finalized_user_content = '\n'.join(
                s.strip() for s in turn.finalized_user_segments if s.strip()
            )
...
async def get_realtime_typed_turn_metadata(request, chat_id: str) -> tuple[dict, str]:
    ...
    user_message_id = str(
        metadata.get('user_message_id') or metadata.get('parent_message_id') or ''
    )
