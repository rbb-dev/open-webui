...
class RealtimeSession:
    ...
    ws: Any | None = None  # websockets.ClientConnection at runtime
    ws_task: asyncio.Task | None = None
    event_emitter: Callable[..., Any] | None = None
    event_emitter_factory: Callable[..., Any] | None = None
    event_caller_factory: Callable[..., Any] | None = None
    ...
    pending_tool_response_create: bool = False
    pending_flush_response_create: bool = False
    last_user_activity: float = field(default_factory=time.monotonic)
    user_idle_timeout_task: asyncio.Task | None = None
    idle_checkin_task: asyncio.Task | None = None
    checkin_suppressed: bool = False
    pending_transient_response_kind: str | None = None
    transient_response_ids: set[str] = field(default_factory=set)
    active_response_id: str = ''
    cancel_requested_response_id: str = ''
    _chat_creation_lock: asyncio.Lock = field(default_factory=asyncio.Lock)
    # Serializes concurrent pending-text flush tasks on this session so
    # ws.send writes and Redis LPOP draining do not interleave across
    # parallel HTTP routes calling notify_pending_text.
    _flush_lock: asyncio.Lock = field(default_factory=asyncio.Lock)

    @property
    def is_ready(self) -> bool:
        return self.sideband_connected and self.context_injected and self.pending_messages_flushed
    ...
    def lookup_event_id(self, event_id: str) -> str | None:
        return self.event_id_cache.get(event_id)

    def touch_activity(self) -> None:
        self.last_activity = time.time()

    async def send_event(self, event_type: str, payload: dict | None = None, context: str | None = None) -> None:
        if not self.ws:
            return
    ...
    async def apply_setting(self, setting: str, value, sio=None) -> None:
        if setting == 'voice':
            await self._apply_voice_setting(value, sio)
            return
        if setting == 'context':
            self.config.context_enabled = bool(value)
            return
        session_update = self._build_session_update(setting, value)
        if session_update is None:
            return
        await self.send_event('session.update', {'session': session_update}, f'setting:{setting}')

    async def _apply_voice_setting(self, value, sio) -> None:
        ...
        # Drop any queued text-handoff messages — they were targeted at the
        # previous voice; the new sideband session will re-bootstrap fresh.
    ...
    def _build_session_update(self, setting: str, value) -> dict | None:
        if setting == 'speed':
            return self._build_speed_update(value)
        if setting == 'max_tokens':
            return self._build_max_tokens_update(value)
        if setting == 'noise_reduction':
            return {'audio': {'input': {'noise_reduction': {'type': value} if value else None}}}
        if setting == 'vad_type':
            return self._build_vad_type_update(value)
        return None

    def _build_speed_update(self, value) -> dict | None:
        speed = float(value)
        if not math.isfinite(speed):
            log.warning('Rejected non-finite speed value: %s', value)
            return None
        return {'audio': {'output': {'speed': speed}}}

    def _build_max_tokens_update(self, value) -> dict | None:
        if str(value).strip().lower() in {'inf', '-1', ''}:
            return {'max_output_tokens': 'inf'}
        tokens = int(value)
    ...
    def _build_vad_type_update(self, value) -> dict:
        turn_detection = None
        ...
        return {'audio': {'input': {'turn_detection': turn_detection}}}

    async def _teardown_sideband(self) -> None:
        current_task = asyncio.current_task()
        if self.ws_task and not self.ws_task.done() and self.ws_task is not current_task:
            self.ws_task.cancel()
            try:
                await self.ws_task
...
class SessionManager:
    def __init__(self):
        ...
        # Strong references to in-flight session-end background tasks so they
        # survive GC and can be awaited on shutdown rather than dropped
        # mid-flight.
        self._end_tasks: set[asyncio.Task] = set()
        self.cleanup_turns: Callable[..., Awaitable[None]] | None = None
        self.session_end_background_tasks: Callable[[RealtimeSession], Awaitable[None]] | None = None

    def get_session(self, session_id: str) -> RealtimeSession | None:
        return self._sessions.get(session_id)

    def get_session_by_chat(self, chat_id: str) -> RealtimeSession | None:
    ...
    async def create_session(
        ...
        if existing_chat_session and existing_chat_session.session_id != session_id:
            await self.teardown_session(existing_chat_session.session_id, reason='replaced')
    ...
    async def teardown_session(self, session_id: str, reason: str = 'user', sio=None) -> RealtimeSession | None:
        ...
        if session.mcp_clients:
            from open_webui.realtime.tool_runtime import cleanup_realtime_mcp_clients

            try:
                await cleanup_realtime_mcp_clients(session.mcp_clients)
            finally:
                session.mcp_clients = {}
        ...
        if reason not in ('replaced',) and self.session_end_background_tasks:
            ...
            def _on_end_task_done(t: asyncio.Task) -> None:
                self._end_tasks.discard(t)
                if not t.cancelled() and t.exception():
                    log.error('Session-end background task failed: %s', t.exception())

            task.add_done_callback(_on_end_task_done)
    ...
    async def drain_end_tasks(self, timeout: float = 10.0) -> None:
        """Await any in-flight session-end background tasks. Used on shutdown
        to avoid losing title/tag/follow-up generation mid-write."""
        tasks = list(self._end_tasks)
        if not tasks:
            return
        try:
            await asyncio.wait_for(asyncio.gather(*tasks, return_exceptions=True), timeout=timeout)
        except TimeoutError:
            log.warning('Session-end task drain timed out after %.1fs with %d still running', timeout, len(tasks))
...
async def maybe_emit_ready(session: RealtimeSession, sio) -> None:
    if session.ready_emitted or not session.is_ready or session.await_bootstrap_replay_response:
        return
...
async def end_session(session: RealtimeSession, sio, reason: str, message: str | None = None) -> None:
...
async def _user_idle_timeout_loop(session: RealtimeSession, sio, timeout_seconds: int) -> None:
    try:
...
async def _idle_checkin_loop(session: RealtimeSession, sio, interval_seconds: int) -> None:
    try:
...
def _schedule_user_idle_timeout(session: RealtimeSession, sio) -> None:
    ...
    if timeout_seconds <= 0:
        return
    session.user_idle_timeout_task = asyncio.create_task(_user_idle_timeout_loop(session, sio, timeout_seconds))
...
def _schedule_idle_checkin(session: RealtimeSession, sio) -> None:
    ...
    if (
        ...
        or session.checkin_suppressed
        or not session.ws
        or session.state not in ('active', 'connecting', 'reconnecting')
    ):
        return
    session.idle_checkin_task = asyncio.create_task(_idle_checkin_loop(session, sio, interval_seconds))
