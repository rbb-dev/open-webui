...
async def should_route_chat_to_realtime(request: Any, model: dict) -> bool:
    return await model_uses_realtime(request, model) and not getattr(request.state, 'direct', False)
...
async def route_chat_completion_to_realtime(request: Any, form_data: dict, user: Any):
    ...
    if get_user_id_from_session_pool(session_id) != user.id:
        raise Exception('Realtime handoff: session_id does not belong to the requesting user.')

    user_message = metadata.get('user_message') or metadata.get('parent_message') or {}
    if message_item is None:
        # Saved chats send no conversation, only the system prompt (history is
        # loaded from the DB), so the typed turn is metadata.user_message.
        content = user_message.get('content', '')
    user_message_id = metadata.get('user_message_id') or metadata.get('parent_message_id') or user_message.get('id', '')
    ...
    handoff = RealtimeTurnHandoff(
        ...
        tree_parent_id=user_message.get('parentId') or '',
