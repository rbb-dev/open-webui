...
async def _prepare_realtime_session_runtime(
    *,
...
) -> tuple[RealtimeSession | None, SessionConfig | None]:
    ...
    if event_emitter_factory:
        event_emitter = await event_emitter_factory(
    ...
    if app:
        ...
        user_model = await Users.get_user_by_id(user['id'])
    ...
    if app and user_model:
        try:
            tool_request, resolved_tools, mcp_clients = await resolve_realtime_tools(
                app=app,
                user=user_model,
                model=model_obj,
                chat_id=chat_id,
                ...
                auth_context=RealtimeToolAuthContext(
                    ...
                    oauth_session_id=(key_data or {}).get('oauth_session_id', ''),
                ),
                event_emitter=event_emitter,
            )
...
async def start_realtime_session(  # noqa: C901 — binding-check + prepare + emit pipeline; sequential validation steps
    sid,
    user,
    model_id,
    chat_id,
    ...
    if key_data:
        ...
        for field_name, expected_value in expected.items():
            ...
            # (empty or placeholder prefix at negotiate time, real id at start time)
            if field_name == 'chat_id':
                expected_str = str(expected_value or '')
                if not expected_str or expected_str.startswith('local:') or expected_str.startswith('channel:'):
                    continue
            if actual_value != expected_value:
                log.warning(
                    'realtime:start rejected: bootstrap binding mismatch sid=%s field=%s',
                    sid,
                    field_name,
                )
                await sio.emit(
    ...
    allowed_settings = [key for key, value in rt_perms.items() if value and key in SUPPORTED_REALTIME_SETTINGS]
    await sio.emit(
        'realtime:session',
        realtime_event_payload(
            call_id=call_id,
    ...
    if call_id and session.ephemeral_key:
        base_url = str(await Config.get('audio.realtime.api_base_url', ''))
        if base_url:
...
async def orphan_sweep_loop(sio) -> None:
    """Background task that runs every 60s to clean up orphaned sessions."""
    while True:
        try:
            await asyncio.sleep(60)
            session_timeout = int(await Config.get('audio.realtime.session_timeout', 180))
            if session_timeout > 0:
                ...
                for session in session_manager.all_sessions():
                    ...
                    if elapsed > session_timeout:
                        log.warning(
                            'Orphan sweep: tearing down idle session %s (idle %.0fs > %ds)',
                            session.session_id,
                            elapsed,
                            session_timeout,
                        )
...
async def _prepare_typed_realtime_turn(
    ...
    *,
    ...
    parent_message, assistant_message_id = await get_realtime_typed_turn_metadata(request, session.chat_id)
    ...
    if parent_message.get('id'):
        ...
        parent_id = ''
        existing_chat = await Chats.get_chat_by_id(session.chat_id)
