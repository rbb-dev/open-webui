...
async def model_uses_realtime(
    request: Any,
    model: Any,
    *,
    require_realtime_engine: bool = True,
) -> bool:
    if require_realtime_engine:
        engine_and_key = await Config.get_many('audio.realtime.engine', 'audio.realtime.api_key')
        if engine_and_key.get('audio.realtime.engine', '') != 'openai':
            return False
        if not engine_and_key.get('audio.realtime.api_key', ''):
            return False
    ...
    whitelist = list(await Config.get('audio.realtime.models', []) or [])
    if not whitelist:
        return True
