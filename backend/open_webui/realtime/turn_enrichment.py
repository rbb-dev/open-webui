...
add_memory_context, chat_web_search_handler, chat_completion_files_handler,
...
async def enrich_realtime_text_turn(  # noqa: C901 — sequential pipeline mirroring process_chat_payload's voice-relevant subset; each step is short and decomposing would require passing 6+ locals through helper signatures
    ...
    # A saved chat's request carries only the system prompt; its history, including
    # this turn's user message, is loaded from the DB with that prompt kept first.
    ...
    if features.get('memory') and not native_fc and await Config.get('memories.system_context.enable'):
        try:
