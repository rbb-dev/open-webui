...
async def build_session_config() -> SessionConfig:
    """Build a SessionConfig snapshot from the current app config."""
    config = await read_realtime_config()
...
def apply_user_realtime_settings(config: SessionConfig, user_obj: Any) -> SessionConfig:  # noqa: C901 — flat per-setting overrides; decomposition would split simple `if user_settings.get(...): config.x = ...` lines for no clarity win
    if not config:
        return config
    ...
    if isinstance(semantic_vad_eagerness, str) and semantic_vad_eagerness in REALTIME_SEMANTIC_VAD_EAGERNESS:
        config.semantic_vad_eagerness = semantic_vad_eagerness

    server_vad_threshold = _coerce_float(realtime_settings.get('serverVadThreshold'), minimum=0.0, maximum=1.0)
    if server_vad_threshold is not None:
        config.server_vad_threshold = server_vad_threshold

    server_vad_silence_duration_ms = _coerce_int(realtime_settings.get('serverVadSilenceDurationMs'), minimum=0)
    if server_vad_silence_duration_ms is not None:
        config.server_vad_silence_duration_ms = server_vad_silence_duration_ms

    server_vad_prefix_padding_ms = _coerce_int(realtime_settings.get('serverVadPrefixPaddingMs'), minimum=0)
    ...
    if isinstance(noise_reduction, str) and noise_reduction in REALTIME_NOISE_REDUCTION_TYPES:
    ...
    vad_interrupt_response = _coerce_bool(realtime_settings.get('vadInterruptResponse'))
...
def build_ephemeral_key_body(config: SessionConfig, model: str) -> dict:  # noqa: C901 — serial config-translation builder mapping OWUI config keys to OpenAI session JSON; each branch is independent and short
    ...
    if config.max_response_output_tokens:
        try:
            ...
            session['max_output_tokens'] = 'inf' if value in {'inf', '-1'} else int(value)
    ...
    if config.truncation_strategy == 'disabled':
        ...
        if config.truncation_token_limit:
            try:
                truncation['token_limits'] = {'post_instructions': int(config.truncation_token_limit)}
            except (ValueError, TypeError):
                pass
...
async def mint_realtime_client_secret(
    ...
    async with aiohttp.ClientSession(timeout=timeout, trust_env=True) as http:
        ...
        if response.status >= 400:
            ...
            error_preview = f'{error_body[:500]}...' if len(error_body) > 500 else error_body
            log.error(f'mint_realtime_client_secret failed: {response.status} {error_preview}')
