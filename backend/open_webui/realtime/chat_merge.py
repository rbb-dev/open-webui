...
def merge_chat_payload(existing_chat: dict, incoming_chat: dict) -> dict:
    ...
    deleted_message_ids = {
        mid for mid in incoming_history.get('deletedMessageIds', []) or [] if isinstance(mid, str) and mid
    }
        ...
        if deleted_message_ids:
            for message in merged_messages.values():
                if isinstance(message, dict) and isinstance(message.get('childrenIds'), list):
                    message['childrenIds'] = [
                        cid for cid in message['childrenIds'] if isinstance(cid, str) and cid not in deleted_message_ids
                    ]
