...
async def claim_start(
    ...
    """Claim ownership of a voice session.

    Returns the record with handoff token, or None if already owned by another.
    """
    now = time.time()
...
async def get_record(voice_session_id: str) -> OwnershipRecord | None:
...
def get_record_by_chat(chat_id: str) -> OwnershipRecord | None:
...
def cleanup_ended_records(max_age_seconds: float = 300.0) -> int:
    ...
    try:
        for key in list(_registry.keys()):
            ...
            if data and data.get('state') == 'ended' and data.get('updated_at', 0) < cutoff:
                try:
                    del _registry[key]
                    removed += 1
                except KeyError:
