    ...
    *,
    ...
    if form_data.system_prompt:
        if await has_permission(user.id, 'chat.system_prompt', request.app.state.config.USER_PERMISSIONS):
            ...
            form = apply_system_prompt_to_body(form_data.system_prompt, form, metadata_vars, user, replace=True)
        else:
            log.warning(
    ...
    if chat_id and not chat_id.startswith('local:'):
        ...
        if folder_id:
            ...
            if folder and folder.data and 'system_prompt' in folder.data:
                form = apply_system_prompt_to_body(folder.data['system_prompt'], form, metadata_vars, user)

    if model_system:
        form = apply_system_prompt_to_body(model_system, form, metadata_vars, user)
