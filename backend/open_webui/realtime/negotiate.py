...
async def execute_negotiate(  # noqa: C901 — sequential HTTP flow: validate -> mint -> SDP exchange -> persist
    *,
    request: Any,
    form_data: Any,
    user: Any,
) -> dict:  # noqa: C901 — sequential HTTP flow: validate -> mint -> SDP exchange -> persist; splitting would force passing 8+ locals through helper signatures
    ...
    api_key = await Config.get('audio.realtime.api_key', '')
    if not api_key:
        raise HTTPException(
    ...
    base_url = await Config.get('audio.realtime.api_base_url', 'https://api.openai.com/v1')
    ...
    if not BYPASS_MODEL_ACCESS_CONTROL and (user.role != 'admin' or not BYPASS_ADMIN_ACCESS_CONTROL):
        try:
            await check_model_access(user, models[model])
    ...
    if not await model_uses_realtime(request, model_obj):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    model_system = await get_realtime_model_system_prompt(model)
    ...
    if form_data.system_prompt:
        if await has_permission(user.id, 'chat.system_prompt', await Config.get('user.permissions', {})):
            form['messages'] = [{'role': 'system', 'content': form_data.system_prompt}]
            form = await apply_system_prompt_to_body(form_data.system_prompt, form, metadata_vars, user, replace=True)
        else:
            system_prompt_dropped = True
            log.warning(
                'Ignoring realtime system prompt override without permission user=%s chat=%s model=%s',
                user.id,
                chat_id or '<none>',
                model,
            )
    ...
    if chat_id and not chat_id.startswith('local:') and not chat_id.startswith('channel:'):
        ...
        if folder_id:
            ...
            if folder and folder.data and 'system_prompt' in folder.data:
                form = await apply_system_prompt_to_body(folder.data['system_prompt'], form, metadata_vars, user)

    if model_system:
        form = await apply_system_prompt_to_body(model_system, form, metadata_vars, user)

    final_system = get_system_message(form.get('messages', []))
    config.system_instructions = final_system.get('content', '') if final_system else ''
    ...
    try:
        ...
        if call_id:
            ...
            await store_ephemeral_key(
                ...
                {
                    **bootstrap_binding,
                    'expires_at': mint_result.get('secret_expires_at') or mint_result.get('token_expires_at'),
                    'session_token': getattr(getattr(request.state, 'token', None), 'credentials', ''),
            ...
            response['warnings'] = [
                {
                    ...
                    'message': 'Custom system prompt was ignored — you do not have permission to override it.',
                },
            ]
        return response
