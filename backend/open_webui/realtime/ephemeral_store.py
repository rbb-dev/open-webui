...
def _purge_stale_ephemeral_keys() -> None:
    ...
    now = time.time()
    stale_keys = [key for key, value in _ephemeral_key_store.items() if _ephemeral_key_expiry(value) <= now]
...
async def pop_ephemeral_key(call_id: str) -> dict | None:
    """Pop and return the full key entry for a call_id, or None."""
    entry = None

    if WEBSOCKET_MANAGER == 'redis':
        raw_entry = await _ephemeral_key_store.execute_command('GETDEL', _ephemeral_key_store_key(call_id))
        if raw_entry:
