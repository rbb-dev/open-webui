...
from open_webui.realtime.session_state import cancel_checkin_timer, restart_checkin_timer_only
...
def register_realtime_socket_handlers(
    *,
    ...
    @sio.on('realtime:stop_checkin')
    async def realtime_stop_checkin(sid, data=None):
        try:
            payload = RealtimeCallPayload.model_validate(data or {})
        except ValidationError:
            return
    ...
    @sio.on('realtime:start_checkin')
    async def realtime_start_checkin(sid, data=None):
        try:
            payload = RealtimeCallPayload.model_validate(data or {})
        except ValidationError:
            return
