...
async def pop_one_pending_text(session_id: str) -> dict | None:
    """Pop one pending text message from the store (FIFO).

    Returns None if the store is empty for that session_id.
    In Redis mode uses LPOP so the message is consumed atomically —
    if the caller fails to send it, it is gone from Redis.
    """
    if _redis_client is not None:
        key = f'{_KEY_PREFIX}{session_id}'
        raw = await _redis_client.lpop(key)
        if not raw:
            return None
        try:
...
async def drain_pending_texts(session_id: str) -> list[dict]:
    """Drain all pending text messages for *session_id*.
...
async def notify_pending_text(session_id: str) -> None:
    ...
    Multi-worker mode publishes on a Redis channel and returns immediately.
    Single-worker mode dispatches the flush as a background task so the
    HTTP request handler that just pushed the pending text isn't blocked
    waiting on the WS round-trip.
    """
    if _redis_client is not None:
        if hasattr(_redis_client, 'nodes_manager'):
            await _redis_client.execute_command('PUBLISH', _NOTIFY_CHANNEL, session_id)
        else:
    ...
    if session and session.is_ready and not session.assistant_responding and not session.tool_executing and session.ws:
        from open_webui.realtime.events import fire_and_forget
        from open_webui.realtime.sideband import _flush_pending_to_ws
        from open_webui.socket.main import sio

        fire_and_forget(_flush_pending_to_ws(session, [], 'immediate_pending', sio=sio, drain_redis=True))
