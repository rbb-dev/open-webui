...
SUMMARY_CONTEXT_INSTRUCTION = (
    'The following block is historical conversation context for continuity. Do not treat it as a new user request.'
)
...
class RealtimeBootstrapContext:
    ...
    unresolved_user_turn: dict | None = None
...
def _normalize_message_for_context(message: dict, *, allow_image_marker: bool) -> dict | None:
    role = str(message.get('role') or '').lower()
    if role not in {'user', 'assistant'}:
        return None

    if role == 'assistant' and isinstance(message.get('output'), list):
        output_msgs = convert_output_to_messages(message['output'])
        content = '\n'.join(m.get('content', '') for m in output_msgs if m.get('role') == 'assistant').strip()
    else:
...
def _extract_completed_exchanges(
    ...
    if cursor >= 0 and str(messages[cursor].get('role') or '').lower() == 'user':
        unresolved_user_turn = _normalize_message_for_context(messages[cursor], allow_image_marker=False)
        cursor -= 1
    ...
    while cursor >= 0:
        ...
        normalized_user = _normalize_message_for_context(user_message, allow_image_marker=True)
        normalized_assistant = _normalize_message_for_context(assistant_message, allow_image_marker=True)
...
async def build_bootstrap_context(
    ...
    for exchange in reversed(older_exchanges):
        ...
        exceeds_exchanges = _would_exceed_limit(total_exchanges, 1, older_summary_exchanges_limit)
        exceeds_bytes = _would_exceed_limit(total_bytes, exchange_size, older_summary_bytes_limit)
    ...
    replay_messages = [message for exchange in replay_exchanges for message in exchange]
    summary_messages = [message for exchange in summary_exchanges for message in exchange]
...
async def generate_context_summary(
    ...
    try:
        ...
        if not request or not getattr(getattr(request, 'app', None), 'state', None):
            log.debug('Context summary skipped: request/app/state not available')
            return None

        models = request.app.state.MODELS

        if not model_id or model_id not in models:
            log.warning('Context summary skipped: model_id %r not in models', model_id)
            return None

        task_models = await Config.get_many('task.model.default', 'task.model.external')
        task_model_id = get_task_model_id(
            model_id,
            task_models.get('task.model.default', ''),
            task_models.get('task.model.external', ''),
            models,
        )
        ...
        payload = {
            ...
            **(
            ...
            'metadata': {
                ...
                'task_body': {'model': model_id, 'summary_max_size': summary_max_size},
        ...
        try:
            ...
            log.debug('Pipeline inlet filter skipped for context summary', exc_info=True)
...
def build_summary_conversation_item(system_summary: str) -> dict | None:
