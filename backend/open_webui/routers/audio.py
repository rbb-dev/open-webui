...
class AudioConfigUpdateForm(BaseModel):
    tts: TTSConfigForm
    stt: STTConfigForm
    rt: RealtimeConfigForm | None = None
...
async def get_realtime_client_config(request: Request, user=Depends(get_verified_user)):
    ...
    return await build_realtime_client_config(request)
...
async def get_audio_config(request: Request, user=Depends(get_admin_user)):
    return {
        ...
        'rt': await build_realtime_admin_config(),
    }
...
async def update_audio_config(request: Request, form_data: AudioConfigUpdateForm, user=Depends(get_admin_user)):
    ...
    if form_data.rt is not None:
        await apply_realtime_admin_config(form_data.rt)
